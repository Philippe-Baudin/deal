...
else // donc il est connecté, ce n'est pas un admin, et il n'envisage pas de supprimer ou modifier une annonce
	{
...
if (!empty($_POST))
	...
	else if (isset ($description_courte) && !preg_match ("/^[- ²,;:!?.\/€%*+()\"'\n\r&\_a-zA-Z0-9ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖÙÚÛÜÝÞàáâãäåæçèéêëìíîïðñòóôõöùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮůŰűŲųŴŵŶŷŸŹźŻżŽžſ]*$/u", $description_courte))
		$contenu .= '<div class="alert alert-danger">Il y a des caractères invalides dans la description courte.</div>';
	...
	else if (isset ($description_longue) && !preg_match ("/^[- ²,;:!?.\/€%*+()\"'\n\r&\_a-zA-Z0-9ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖÙÚÛÜÝÞàáâãäåæçèéêëìíîïðñòóôõöùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮůŰűŲųŴŵŶŷŸŹźŻżŽžſ]*$/u", $description_longue))
		$contenu .= '<div class="alert alert-danger">Il y a des caractères invalides dans la description longue.</div>';
	...
	else if (isset ($pays) && !preg_match ("/^[- .a-zA-Z0-9ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖÙÚÛÜÝÞàáâãäåæçèéêëìíîïðñòóôõöùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮůŰűŲųŴŵŶŷŸŹźŻżŽžſ]*$/u", $pays))

	if (!isset ($ville) || strlen($ville) < 1 || strlen ($ville) > 100)
		$contenu .= '<div class="alert alert-danger">La ville n\'existe pas.</div>';
	else if (isset ($ville) && !preg_match ("/^[- .a-zA-Z0-9ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖÙÚÛÜÝÞàáâãäåæçèéêëìíîïðñòóôõöùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮůŰűŲųŴŵŶŷŸŹźŻżŽžſ]*$/u", $ville))

	if (!isset ($adresse) || strlen($adresse) < 4)
		$contenu .= '<div class="alert alert-danger">L\'adresse est invalide.</div>';
	else if (isset ($adresse) && !preg_match ("/^[- .,\/#()a-zA-Z0-9ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖÙÚÛÜÝÞàáâãäåæçèéêëìíîïðñòóôõöùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮůŰűŲųŴŵŶŷŸŹźŻżŽžſ]*$/u", $adresse))
	...
	if (empty($contenu))
		{
		if (empty($id))
			$stringRequete = "INSERT INTO annonce VALUES (:id, :titre, :description_courte, :description_longue, :prix, '', :pays, :ville, :adresse, :code_postal, :membre_id, :categorie_id, NOW())";
		else
			$stringRequete = "UPDATE annonce SET titre=:titre, description_courte=:description_courte, description_longue=:description_longue, prix=:prix, pays=:pays, ville=:ville, adresse=:adresse, code_postal=:code_postal, membre_id=:membre_id, categorie_id=:categorie_id, date_enregistrement=".((isset($date_enregistrement)&& $date_enregistrement != '')?':date_enregistrement':'NOW()')." WHERE id=:id";
		...
		if ($resultat)
			...
			if (empty($id))
				{
				// Le plus grand id, donc celui qu'on vient de créer
		...
		<form id="formulaire" method="post" action="gestion_annonces.php?page=<?php echo $_SESSION['page']??0?>" enctype=multipart/form-data>
			...
			<div class="form-row">
				...
				<div class="form-group col-md-6">
					...
					<div>
						<?php
							// Afficher la photo déjà présente dans l'annonce
							if (!empty($photo)) 
