...
$resultat=executerRequete("SELECT pseudo, COUNT(note) nombreAvis, AVG(note) moyenne FROM membre LEFT JOIN note ON membre.id=note.membre_id2 GROUP BY membre.id ORDER BY moyenne DESC, nombreAvis DESC LIMIT 5");
...
$contenu .=             '<div class="row">';
$contenu .=                 '<div class="col-sm-auto">';
for ($i=0; $i<5 && $i<sizeof($tableAvis); $i++)
	$contenu .=                 '<p>'.($i+1).' - '. $tableAvis[$i]['pseudo'].' :<p>';
$contenu .=                 '</div>';
$contenu .=                 '<div class="col-sm">';
for ($i=0; $i<5 && $i<sizeof($tableAvis); $i++)
	$contenu .=                 '<p>'.noteEnEtoiles($tableAvis[$i]['moyenne']).' ('.$tableAvis[$i]['nombreAvis'].' avis)</p>';
$contenu .=                 '</div>';
$contenu .=             '</div>';
$contenu .=         '</div>'; // class="col-sm-6">';
